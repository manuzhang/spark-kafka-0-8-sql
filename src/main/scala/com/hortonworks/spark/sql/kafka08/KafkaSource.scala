...
case class KafkaSource(
    ...
    val metadataLog = new HDFSMetadataLog[KafkaSourceOffset](sqlContext.sparkSession, metadataPath) {
      ...
      override def deserialize(in: InputStream): KafkaSourceOffset = {
        ...
        if (content(0) == 'v') {
          ...
          if (indexOfNewLine > 0) {
            val version = SparkSqlUtils.parseVersion(this,
              content.substring(0, indexOfNewLine), KafkaSource.VERSION)
            KafkaSourceOffset(content.substring(indexOfNewLine + 1))
          } else {
  ...
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    val rdd = KafkaUtils.createRDD[
      ...
      DefaultDecoder,
      DefaultDecoder,
      Row](sc, kafkaParams, offsetRanges, leaders, messageHandler).map { row =>
      InternalRow.fromSeq(
        Seq(
          row.get(0),
          row.get(1),
          UTF8String.fromString(row.get(2).asInstanceOf[String]),
          row.get(3),
          row.get(4)
        )
      )
    ...
    SparkSqlUtils.createStreamingDataFrame(sqlContext, rdd, schema)
  }
